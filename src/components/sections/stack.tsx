...
import type { StackCategory, StackItem } from "@/types/stack";

export default function Stack() {
    const { content, dict } = useLanguage();

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const cnt = content as any;
    const categories: StackCategory[] = cnt.stackCategories || [
        {
            title: dict.frontendStack,
            items: cnt.stack?.frontend || [],
        },
        {
            title: dict.backendStack,
            items: cnt.stack?.backend || [],
        },
        {
            title: dict.databaseStack,
            items: cnt.stack?.database || [],
        },
        {
            title: dict.toolsStack,
            items: cnt.stack?.tools || [],
        },
    ];

    return (
        <section id="stack" className="w-full bg-background text-foreground overflow-hidden relative py-16 md:py-24 lg:py-32 xl:py-40 2xl:py-36">
                                        ...
                                        <HoverCard key={item.name} openDelay={50} closeDelay={50}>
                                            ...
                                            >
                                                ...
                                                {item.icon ? (
                                                    ...
                                                    <div className="relative p-3.5 rounded-xl bg-secondary/50 ring-1 ring-border/50 shadow-inner flex items-center justify-center">
